...
export async function findVerificationByToken(token) {
    const [rows] = await pool.query(`SELECT * FROM email_verifications WHERE token = ?`,[token]);
    if (rows.length === 0) {
        return null;
    }
    return rows[0];
...
export async function markVerificationAsVerified(id) {
    const [result] = await pool.query(`UPDATE email_verifications SET verified_at = CURRENT_TIMESTAMP WHERE id = ? AND verified_at IS NULL`,[id]);
